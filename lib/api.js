...
export const api = {
  ...
  // ---- Reports ----
  getMyReports: () => apiRequest("/reports/me"),
  getChildReports: (studentId) => apiRequest(`/reports/child/${studentId}`),
  generateMyReportNow: () => apiRequest("/reports/generate", { method: "POST" }),

  // ---- Subscriptions ----
  getPlans: () => apiRequest("/subscriptions/plans"),
  createPaymentIntent: (planId) =>
    apiRequest("/subscriptions/create-payment-intent", { method: "POST", body: { planId } }),
